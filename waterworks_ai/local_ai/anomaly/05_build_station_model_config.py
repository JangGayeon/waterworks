...
def classify_sensor_type(sensor_name: str) -> str:
    ...
    # pressure
    pressure_keywords = [
        "PIT", "PT", "압력", "유입압력", "유출압력", "토출압력", "흡입압력", "토출", "흡입"
    ]
    ...
    # level
    level_keywords = ["LT", "LEVEL", "수위", "배수지수위", "저수지수위"]
    ...
    # inverter
    if "인버터" in raw or "INV" in s or "주파수" in raw:
        return "inverter"
...
def select_features_for_station(
    ...
    # 이미 usable 판정된 센서만 사용
    usable = station_df[
        (station_df["usable"] == True) &
        (station_df["all_zero"] == False)
    ].copy()

    # other도 완전히 버리지 않고 fallback 후보로 남긴다
    ...
    # 최소 feature 보장: usable 센서 중에서 추가 보충
    ...
    # 그래도 0이면 other 포함해서 한 번 더 완화
    if len(selected_df) == 0:
        fallback_df = station_df[
            (station_df["usable"] == True) &
            (station_df["all_zero"] == False)
        ].copy()
        ...
        selected_df = fallback_df.head(min_features).copy()
